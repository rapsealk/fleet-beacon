...
from src.fleet_beacon.unit.service import delete, get, get_all, get_or_create, update

router = APIRouter()


@router.post("", response_model=UnitRead, status_code=status.HTTP_201_CREATED)
async def create_robot(*, db_session: Session = Depends(get_db), robot_in: UnitCreate):
